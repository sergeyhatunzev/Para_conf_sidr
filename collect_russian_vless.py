...
with open(sidr_output, 'w', encoding='utf-8') as f:
    ...
    try:
        ...
        print("Пропускаем дедупликацию Goida.")

    # Первая дедупликация: убираем дубликаты Goida из новых конфигов
    clean_configs = []  # новые уникальные относительно Goida
    duplicates_goida = 0
    if goida_bases:
        ...
        print(f"\nДедупликация Goida завершена. Удалено дубликатов: {duplicates_goida}")
        print(f"Осталось новых уникальных: {len(clean_configs)}")
    else:
        clean_configs = matched_with_ip[:]
        print("Дедупликация Goida пропущена.")

    # Вторая часть: скачиваем старый clean_vless.txt, объединяем и дедуплицируем
    CLEAN_VLESS_URL = "https://raw.githubusercontent.com/sergeyhatunzev/Para_conf_sidr/refs/heads/main/clean_vless.txt"

    print("\nСкачиваем предыдущий clean_vless.txt для объединения...")
    old_configs = []
    try:
        resp = requests.get(CLEAN_VLESS_URL)
        resp.raise_for_status()
        old_lines = resp.text.splitlines()
        old_configs = [l.strip() for l in old_lines if l.strip().startswith('vless://')]
        print(f"Загружено {len(old_configs)} конфигов из предыдущего clean_vless.txt")
    except Exception as e:
        print(f"Ошибка загрузки предыдущего clean_vless.txt: {e}")
        old_configs = []  # если не загрузилось — просто используем новые

    # Объединяем старые + новые уникальные
    all_configs = old_configs + clean_configs
    print(f"Всего конфигов до финальной дедупликации: {len(all_configs)} "
          f"(старые: {len(old_configs)}, новые уникальные: {len(clean_configs)})")

    # Финальная дедупликация по base_vless
    seen_bases = set()
    final_unique = []
    duplicates_final = 0

    with tqdm(all_configs, desc="Финальная дедупликация (объединение)", unit="конфиг") as pbar:
        for line in pbar:
            base = get_base_vless(line)
            if base in seen_bases:
                ...
                seen_bases.add(base)
                final_unique.append(line)

    # Сохраняем в тот же файл clean_output (перезаписываем старый новыми + уникальными)
    with open(clean_output, 'w', encoding='utf-8') as f:
        for cfg in final_unique:
            f.write(cfg + '\n')

    print(f"\nОбъединение и дедупликация завершены.")
    print(f"Удалено дубликатов при объединении: {duplicates_final}")
    print(f"Итоговое количество уникальных конфигов: {len(final_unique)}")
    print(f"Сохранено в {clean_output} (файл обновлён)")
